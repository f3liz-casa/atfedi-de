...
- 宛先の解決は、いつも[webfingerの門](/section/fedify/rooms/webfinger/)から。「@handle@host」という名前が、まずactorのURLに引かれる
- 文書は素のJSONではなくJSON-LD。語彙の意味は`@context`が担ぐ——だから船は、辞書ごと運んでいる
- 封蝋(HTTP署名)には新旧二つの作法があって、fedifyの[儀典の間](/section/fedify/rooms/sig/)は相手に合わせて使い分ける
