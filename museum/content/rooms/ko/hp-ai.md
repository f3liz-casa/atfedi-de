...
AI 실험 동이에요. 공개하는 기능은 딱 네 가지 — altText(이미지 대체문), moderation(신고 초벌 읽기), summary, translate예요. Vercel의 ai SDK 위에 세워져 있고, 모델(Anthropic/Google)은 부르는 쪽이 챙겨 와요.

moderation의 벽에는, 또렷하게 적혀 있어요 — 이건 사람 모더레이터를 위한 참고 자료이지, 자동 판결이 아니라고요. 행동 규범 조항마다 확신도와 이유를 붙여 돌려줄 뿐이에요. 판단은, 사람의 몫 그대로예요.
